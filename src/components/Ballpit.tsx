...
const Ballpit: React.FC<BallpitProps> = ({ className = '', followCursor = true, ...props }) => {
  ...
  useEffect(() => {
    ...
    if (!host) return;
    // A fresh canvas per mount: React Strict Mode runs effects twice in development and a disposed
    // WebGL context cannot be reused on the same element.
  ...
  return <div className={`${className} w-full h-full`} ref={hostRef} />;
};
